...
const ALL_ACTIONS: AuditLogAction[] = [
  "login","logout","mission.create","mission.update","mission.delete","mission.assign",
  "dataset.upload","dataset.delete","dataset.replace","analysis.start","analysis.complete",
  "report.generate","report.download","anomaly.comment","anomaly.status_change",
];
...
export default function AuditTrailPage() {
  ...
  function exportCSV(logs: AuditLog[]) {
    const headers = [t("auditTrail.dateCol"), t("auditTrail.userCol"), t("auditTrail.roleCol"), t("auditTrail.actionCol"), t("auditTrail.missionCol"), t("auditTrail.detailsCol")];
  ...
  const countLabel = logs.length === 1
    ? t("auditTrail.countSingular", { count: logs.length })
    : t("auditTrail.countPlural", { count: logs.length });

  const resultsLabel = filtered.length === 1
    ? t("auditTrail.resultsSingular", { count: filtered.length })
    : t("auditTrail.resultsPlural", { count: filtered.length });

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("auditTrail.title")}</h1>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={actionFilter} onValueChange={(v) => setActionFilter(v as AuditLogAction | "all")}>
          <SelectTrigger className="w-[210px]">
            <SelectValue placeholder={t("auditTrail.allActions")} />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={roleFilter} onValueChange={(v) => setRoleFilter(v as UserRole | "all")}>
          <SelectTrigger className="w-[160px]">
            <SelectValue placeholder={t("auditTrail.allRoles")} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t("auditTrail.allRoles")}</SelectItem>
        ...
        {(search || actionFilter !== "all" || roleFilter !== "all") && (
          <Button variant="ghost" size="sm" onClick={() => { setSearch(""); setActionFilter("all"); setRoleFilter("all"); }}>
            {t("auditTrail.reset")}
          </Button>
        )}
        <span className="ml-auto text-sm text-muted-foreground">{resultsLabel}</span>
      </div>
              ...
              {filtered.map((log, idx) => {
                ...
                return (
                  <div key={log.id} className="relative flex gap-4 pl-10">
                    <div className={`absolute left-0 flex h-9 w-9 items-center justify-center rounded-full border-2 border-white shadow-sm ${colorClass}`}>
                      <Shield className="h-3.5 w-3.5" />
                    </div>
                    <Card className={`mb-2 flex-1 ${idx === 0 ? "border-pwc-orange/30" : ""}`}>
                          ...
                          <div className="space-y-0.5">
                            ...
                            {log.mission_name && (
                              <p className="text-[10px] text-muted-foreground/70">
                                {t("auditTrail.missionCol")} : {log.mission_name}
                              </p>
                            )}
                          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      {[t("auditTrail.dateCol"), t("auditTrail.userCol"), t("auditTrail.roleCol"), t("auditTrail.actionCol"), t("auditTrail.missionCol"), t("auditTrail.detailsCol")].map((col) => (
                        <th key={col} className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground">{col}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {filtered.map((log) => (
                      <tr key={log.id} className="hover:bg-muted/20">
                        <td className="whitespace-nowrap px-4 py-2 text-xs text-muted-foreground">{formatDateTime(log.timestamp)}</td>
                        ...
                        <td className="px-4 py-2">
                          <Badge variant="outline" className="text-[10px]">{t(`role.${log.user_role}`)}</Badge>
                        </td>
                        <td className="whitespace-nowrap px-4 py-2">
                          <span className={`rounded px-2 py-0.5 text-[10px] font-medium ${ACTION_ICON_COLOR[log.action] ?? "bg-gray-100 text-gray-600"}`}>
                            {t(`action.${log.action}`)}
                          </span>
                        </td>
                        <td className="px-4 py-2 text-xs text-muted-foreground">{log.mission_name ?? "—"}</td>
                        <td className="max-w-[280px] truncate px-4 py-2 text-xs text-muted-foreground">{log.details}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
  ...
  return (
    <Card>
      <CardContent className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="mt-4 font-medium text-muted-foreground">{t("auditTrail.empty")}</p>
        <p className="mt-1 text-sm text-muted-foreground">{t("auditTrail.emptyDesc")}</p>
      </CardContent>
    </Card>
  );
}
