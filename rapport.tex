...
Ce projet porte sur la détection intelligente d'anomalies financières à partir de données transactionnelles d'un client PwC Tunisie. L'objectif est de construire une chaîne complète et reproductible allant de l'analyse exploratoire des données à la préparation, puis à l'entraînement et à l'évaluation de plusieurs approches de détection.
...
This project focuses on intelligent anomaly detection in financial transactions using client transaction data provided in a PwC Tunisia mission. The goal is to build a reproducible pipeline covering exploratory analysis, data preparation, model training, and evaluation.
...
Le projet s'appuie sur des données transactionnelles client représentant des opérations financières réelles ou anonymisées. Les variables décrivent notamment le type d'opération, le montant, les soldes avant et après transaction, ainsi que plusieurs variables temporelles et comportementales.
